...
async def process_image(message: Message, album: list[Message], tg_user: TelegramUser, state: FSMContext) -> None:
    """Handler for uploading images to database by admin users only."""
...
async def process_add_image(message: Message, album: list[Message], tg_user: TelegramUser, state: FSMContext) -> None:

    await state.set_state(UploadStates.UPLOAD_IMAGE)


@router.message(UploadStates.CONFIRMATION, F.text == reply.ConfirmUploadChoices.FINISH)
async def process_finish(message: Message, album: list[Message], tg_user: TelegramUser, state: FSMContext) -> None:
    pass


@router.message(UploadStates.CONFIRMATION, F.text == reply.ConfirmUploadChoices.CANCEL)
async def process_cancel(message: Message, album: list[Message], tg_user: TelegramUser, state: FSMContext) -> None:
    pass
