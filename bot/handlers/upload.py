...
async def process_upload_confirm(message: Message, current_user: TelegramUser, state: FSMContext) -> None:
...
async def process_upload_finish(message: Message, current_user: TelegramUser, state: FSMContext) -> None:
    pass


@router.message(UploadStates.CONFIRMATION, F.text == reply.ConfirmUploadChoices.CANCEL)
async def process_upload_cancel(message: Message, current_user: TelegramUser, state: FSMContext) -> None:
    pass
